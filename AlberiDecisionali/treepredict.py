...
def divideset(rows,column,value):
   ...
   split_function=None
    # Se i valori sono numerici il criterio vero
    # prende i dati maggiori del value che gli abbiamo passato
   if isinstance(value,int) or isinstance(value,float):
      split_function=lambda row:row[column]>=value
    # Se i valori non sono numerici il criterio vero
    # prende i dati che hanno lo stesso value dato
   else:
      split_function=lambda row:row[column]==value
...
def uniquecounts(rows):
   results={}
   for row in rows:
      # Il risultato è l'ultima colonna
      r=row[len(row)-1]
...
def giniimpurity(rows):
    # Mi indica la purezza del set, se gli oggetti sono tutti omogenei
    # allora non ho possibilità d'errore, altrimenti se invece ho due possibili risultati
    # l' impurità e' del 50% (con set di 2 elementi), in altri termini vede la probabilità
    # con cui cade un evento nel set poi la va a moltiplicare per le altre probabilità
    # di oggetti diversi da se stesso e somma i valori
  total=len(rows)
...
def mdclassify(observation,tree):
  if tree.results!=None:
    ...
    if v==None:
      ...
      return result
    # Semplice classificazione perchè il valore è presente
    else:
      if isinstance(v,int) or isinstance(v,float):
        if v>=tree.value: branch=tree.tb
        else: branch=tree.fb
...
def buildtree(rows,scoref=entropy):
  ...
  for col in range(0,column_count):
    ...
    for value in column_values.keys():
      ...
      # Calcola la probabilità che si presenti un risultato vero
...
# ------------------------------- FINE del codice del Capitolo 7 --------------------------------
...
# Costruisco l'albero fornendo un file.txt che contiene i dati del dataset
r = createDT(300, "mushroom.txt")

# Misuro la performance di quel dataset
